...
switch (TRUE)
{
  ...
  { # Capturar o valor do código do médico, pesquisar na tabela médicos e montar tabela Campo|Valor exibindo os dados.
    mostraregistro("$_REQUEST[cplivro]");
    printf("<form action='./livexc.php' method='POST'>\n");
  ...
  { # Tratamento da transação de exclusão do registro
    printf("Tratatando a exclusão do registro $_REQUEST[cplivro]<br>\n");
    # construção do comando de atualização.
    $cmdsql="DELETE FROM livros WHERE livros.cplivro='$_REQUEST[cplivro]'";
    $mostrar=FALSE;
    $tenta=TRUE;
    while ( $tenta )
    { # laço de controle de exec da trans.
      ...
      { # trans pode ser concluída e não reiniciar
        ...
        $tenta=FALSE;
        $mostrar=TRUE;
        $mens="Registro com código $_REQUEST[cplivro] excluído!";
      }
      else
      {
